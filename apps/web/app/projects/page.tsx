...
export default function ProjectsPage() {
  ...
  if (loading) return <main style={{ padding: 32 }}>Загрузка проектов...</main>;
  ...
  return (
    <main style={{ padding: 32, background: '#f3f5f9', minHeight: '100vh' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <h1 style={{ marginBottom: 12 }}>Проекты</h1>
        ...
        <table style={{ width: '100%', background: '#fff', borderRadius: 16, borderCollapse: 'collapse', overflow: 'hidden', boxShadow: '0 1px 2px rgba(16,24,40,0.04)' }}>
          <thead>
            <tr style={{ background: '#f8fafc', color: '#667085', textTransform: 'uppercase', fontSize: 12 }}>
              ...
              <th style={{ padding: 16, textAlign: 'left' }}>Проект</th>
              <th style={{ padding: 16, textAlign: 'left' }}>Статус</th>
              <th style={{ padding: 16, textAlign: 'left' }}>Прогресс</th>
            </tr>
          </thead>
          <tbody>
            {projects.map((project) => (
              <tr key={project.id ?? project.code} style={{ borderTop: '1px solid #eef2f6' }}>
                <td style={{ padding: 16, fontWeight: 700 }}>{project.code}</td>
                <td style={{ padding: 16 }}><Link href="/dashboard" style={{ color: '#111827', textDecoration: 'none', fontWeight: 600 }}>{project.name}</Link></td>
                <td style={{ padding: 16 }}>{project.status}</td>
                <td style={{ padding: 16 }}>{project.progress ?? 'N/A'}</td>
              </tr>
            ))}
          </tbody>
        </table>
